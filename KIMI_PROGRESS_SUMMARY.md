...
生成日期：2026-06-07（本版更新于 2026-06-08 22:48）
...
- `cross_profile_rules` 已开始从单纯导入后宫规则扩展到少量非后宫组合：推理导入刑侦法医的案情/证据/技术边界与克系异常调查规则，历史导入军事战争的战役/后勤/政治目标规则，种田经营导入末世资源和秩序建设规则，蒸汽西幻导入推理单元与克系异常规则，避免混合题材只保留单一专项视角。
- 自动分类增加标题 3 倍权重、组合关键词加成和负向关键词抑制，覆盖 `末世之修仙系统`、`三国之种田霸业`、`转生之我在娱乐圈当影后` 等 Kimi v3 边界样例，并减少篮球/系统、娱乐圈/经营等跨类噪声。
- 自动分类继续按 `kimi_v3` 算法报告补强：过滤 `操作系统/文件系统/系统化`、`第四天灾`、`修仙种田`、`兵工厂`、`证据链` 等短词污染；局部否定语境里的关键词不会计分；重复高权重关键词会有小幅封顶加分；默认多标签阈值对低区分度 profile 更谨慎、对高区分度 profile 更宽松；蒸汽西幻已补充 `蒸汽+炼金`、`蒸汽朋克+西幻`、`差分机+蒸汽+炼金矩阵` 组合关键词加成；都市爽文已按 P1-9 降低 `系统/修仙` 共享词权重、提高 `下山/龙王/战神` 独占锚点权重，并继续补充商战主线、军事主线的负向锚点，避免 `系统+商战`、`兵王+战神` 把职场商战或军事战争样例误拉进前三；P2 权重校准已覆盖 `AI`、`边军/骑兵`、`校花/家族/集团/商战`、`技能`、`游戏` 等 Kimi 点名弱信号，同时保留篮球系统、娱乐圈游戏、纯商战等边界回归；`confidence` 已从候选相对占比改为基于分数阈值、领先幅度和证据数量的经验校准值，并新增 `confidence_level` 高/中/低分级。
...
- 低覆盖专项规则已补齐到不少于 12 个审查点：国运增加奖励反噬、战力尺度和直播舆论；幕后流增加马甲数量管理、组织资源成本和暴露后果；模拟器增加结算通胀、现实信息差和重复日志压缩；中式诡异增加规则冲突、仪式代价和污染扩散；都市爽文增加专业能力展示和法治舆论后果；异世界轻小说增加轻小说节奏。
...
- 后宫报告会在女主扫描结果与审核洁度名单不一致时追加 `【交叉验证提示】`，提示“扫描阶段识别到但审核洁度未覆盖”或“审核洁度中出现但扫描女主列表未列出”的角色；匹配时会使用女主别名、核心名和女性角色证据里的 `other_names`，避免简单别名差异误报。
- 后宫报告的交叉验证已覆盖扫描雷点与二审输出一致性：首扫 `issues` 中的严格关系雷点或明确雷点/郁闷点若未出现在 `lei_points/yumen_points/pending_points/rejected_points` 中，会提示“扫描阶段发现但二审输出未覆盖”；二审驳回项会写入 `rejected_points`，避免已驳回问题被误报为遗漏。
- 后宫报告已增加 `【关系图谱】` Mermaid 区块，输出男主到女主/准女主的关系边，并在边标签中展示女主定位、洁度结论和接触等级，便于前端或 Markdown 工具直接渲染人物关系网。
- 后宫报告已增加 `【关键事件时间线】` Markdown 表格，按 chunk 顺序展示女主剧情、男主互动、情感信号、推倒/关系确认、漏女风险、雷点和郁闷点事件，便于快速定位感情线推进和风险节点。
- 通用报告已增加 `【多维度评分】`：输出剧情质量、人物塑造、世界观、节奏把控、文笔水准、情绪调动六维 0-10 分，并附带 `前端评分JSON` 代码块；新 summary 会优先使用模型给出的 `radar_scores`，旧 summary 会按已有字段和角色材料保守估算。
...
- 本地限流作用域默认改为 `auto`：多 API Key 时自动使用 `per_key` 分桶，单 Key 时继续使用 `global`，减少多 key 场景下的无谓串行等待，同时保留 `global/per_key` 手动覆盖。
...
- Web 启动加载 `web_manager_state.json` 失败时不再静默吞掉异常，会写入 `web_manager` warning 日志并继续按空状态/目录同步恢复，便于排查状态文件损坏导致的任务列表异常。
- Web 静态资源、`/files` 和小说文件安全判断已从字符串前缀拼接改为 `os.path.commonpath()` 目录归属校验，避免相邻目录或跨平台路径分隔符导致的路径绕过。
...
- `docker-compose.yml` 显式透传后宫增强、限流作用域、上传限制、JSON 限制、文件流式块大小、目录同步 TTL、输出缓存 TTL、SSE 推送间隔、SSE 同步间隔和 SSE 连接生命周期等 `.env` 运行参数，避免只写在 `.env` 但容器内未生效。
...
- 角色识别、通用/专项剧情扫描、二审和报告阶段已增加 JSON mode 降级，并统一收敛到 `shared_utils.call_json_chat_completion_with_fallback`：首次请求仍使用 `response_format=json_object`，若模型返回内容无法解析为 JSON，或兼容接口直接拒绝 `response_format`，会追加纠错提示并用普通模式重试一次，降低部分 OpenAI 兼容接口 JSON mode 不稳定导致的 chunk 失败率、角色合并失败率、二审判断失败率和报告摘要兜底退化率，同时避免各阶段重复维护降级逻辑。
...
- detail 路径解析已支持显式 `book_name/base_dir/active_detail_path`，可在不依赖全局 `clean_filename` 与 `_ACTIVE_DETAIL_PATH` 的情况下为指定书籍查找角色 detail 文件，继续降低多书并发时的串书风险。
- 最终报告生成与全局补扫学习关键词保存已支持显式书名传入，主流程会传入当前 `clean_filename`，减少报告标题和 learned keyword 元数据对模块级书名全局的隐式依赖。
- 首扫、补扫和 chunk 提交路径已支持显式 `chunk_summaries` 字典；默认路径继续兼容模块级 `CHUNK_SUMMARIES`，显式传入时摘要更新和 checkpoint 保存会写入调用方字典，继续降低多书并发时的摘要串书风险。
- 失败 chunk 诊断记录已支持显式 `chunk_failure_diagnostics` 字典；默认路径继续兼容模块级 `CHUNK_FAILURE_DIAGNOSTICS`，显式传入时失败记录、成功清理和 checkpoint 保存会写入调用方字典，继续降低多书并发时的失败诊断串书风险。
- 首扫中段前情摘要限额已支持显式 `middle_summary_state` 计数器；主流程为每本书创建独立计数状态，默认直接调用路径继续兼容模块级 `_middle_summary_calls`，降低多书并发时摘要补生成额度互相抢占的问题。
...
- 通用/副类型扫描会按小说长度动态提高 `GENERAL_SCAN_MAX_CHUNKS` 的有效上限：100万字内保持 80，100-300万字提高到 120，300-500万字提高到 160，500-1000万字提高到 300，1000万字以上提高到 400；显式配置更高值时尊重配置，配置为 0 时仍表示不限制。原始分块超过预算时会按全书时间线均匀抽样并保留首尾片段，避免千万字长篇只扫前段。
...
- detail 显式书名查找，验证全局 active detail 指向其他书时仍能按传入书名读取正确女主/男主。
- 报告生成显式书名，验证全局 `clean_filename` 与传入书名不一致时优先使用传入值。
- chunk 摘要显式注入，验证提交结果会更新调用方摘要字典而不污染模块级 `CHUNK_SUMMARIES`。
- 失败诊断显式注入，验证失败记录和成功清理只作用在调用方诊断字典，不污染模块级 `CHUNK_FAILURE_DIAGNOSTICS`。
- 中段摘要限额状态显式注入，验证首扫线程块会更新调用方计数器而不污染模块级 `_middle_summary_calls`。
...
- 限流作用域 `auto`，验证单 key 自动解析为 `global`、多 key 自动解析为 `per_key`、显式参数/环境变量/default 的优先级正确，Web 运行配置和前端选择框均支持 `auto`。
...
- 通用角色识别和通用/专项剧情扫描长篇抽样，验证 1000 万字级预算提升到 300、1000 万字以上提升到 400，且超预算时按全书时间线均匀抽样而不是截取前 N 块；主流程落盘测试已覆盖 1000 万字、1000 原始片段抽样为 300 片段的情况。
- 后宫报告交叉验证提示，验证扫描/审核女主名单不一致会提示，别名/核心名一致不会误报；扫描雷点未被二审输出覆盖会提示，已进入 `rejected_points` 的驳回项不会误报。
- 后宫报告 Mermaid 关系图谱，验证输出男主节点、女主节点、关系边、女主定位、洁度标签和接触等级。
...
- 角色识别、通用/专项剧情扫描、二审和报告阶段 JSON mode 降级重试，验证共享 helper 和各阶段薄封装在首次返回非 JSON 或接口拒绝 `response_format` 时会用普通模式追加纠错提示重试一次，且正常 JSON mode 成功路径不变。
...
Kimi v3 建议的 4 个新兴类型已落地：国运/文明对抗流、幕后流/马甲流、模拟器流和中式诡异/规则怪谈。P1-2 指出的 `steampunk_fantasy/harem/urban_power/crime_forensics` 数量不均衡已处理，当前分别为 9/9/9/11 条；此前低于 12 个审查点的 `chinese_weird/mastermind_hidden/nation_fate/simulator/urban_power/isekai_lightnovel` 也已补齐到 12 点。剩余差异主要是 Kimi 原字段名与当前字段命名风格不同，或部分句子已经语义覆盖但不逐字一致；这些属于低风险提示词打磨，不影响当前功能主线。

`cross_profile_rules` 不再只用于“其他分类导入后宫规则”：已配置首批非后宫互补导入，但仍保持克制，只覆盖推理/历史/种田/蒸汽西幻这类混合题材里最容易缺失的专项审查点。后续若真实报告显示某些组合 prompt 过重或误判增加，再按样本回退或细化导入点。

...
生成日期：2026-06-07（本版更新于 2026-06-08 21:55）
...
- 通用/专项剧情扫描已增加 JSON mode 解析失败降级：首次请求仍使用 `response_format=json_object`，若模型返回内容无法解析为 JSON，会追加纠错提示并用普通模式重试一次，降低部分 OpenAI 兼容接口 JSON mode 不稳定导致的 chunk 失败率。
...
- 通用/专项剧情扫描 JSON mode 降级重试，验证首次返回非 JSON 时会用普通模式追加纠错提示重试一次，且正常 JSON mode 成功路径不变。

...
- 自动分类继续按 `kimi_v3` 算法报告补强：过滤 `操作系统/文件系统/系统化`、`第四天灾`、`修仙种田`、`兵工厂`、`证据链` 等短词污染；局部否定语境里的关键词不会计分；重复高权重关键词会有小幅封顶加分；默认多标签阈值对低区分度 profile 更谨慎、对高区分度 profile 更宽松；蒸汽西幻已补充 `蒸汽+炼金`、`蒸汽朋克+西幻`、`差分机+蒸汽+炼金矩阵` 组合关键词加成；都市爽文已按 P1-9 降低 `系统/修仙` 共享词权重、提高 `下山/龙王/战神` 独占锚点权重，并继续补充商战主线、军事主线的负向锚点，避免 `系统+商战`、`兵王+战神` 把职场商战或军事战争样例误拉进前三；`confidence` 已从候选相对占比改为基于分数阈值、领先幅度和证据数量的经验校准值，并新增 `confidence_level` 高/中/低分级。

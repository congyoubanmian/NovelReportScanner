...
class ProfileAndGeneralReportTests(unittest.TestCase):
    ...
    def test_shared_json_parser_rejects_truncated_json_before_repair(self):
        original_import = builtins.__import__
        calls = []
        ...
        try:
            builtins.__import__ = fake_import
            with self.assertRaisesRegex(json.JSONDecodeError, "truncated_json_response"):
                shared_utils.parse_json_object_lenient(
                    '```json\n{"female_characters":[{"name":"辛月","summary":"未闭合"'
                )
        finally:
            builtins.__import__ = original_import
    ...
    def test_harem_character_chunk_downshifts_api_504_by_splitting_text(self):
        class Response:
            status_code = 504
        ...
        old_depth = protagonist.HAREM_SCAN_API_DOWNSHIFT_MAX_DEPTH
        old_min_chars = protagonist.HAREM_SCAN_API_DOWNSHIFT_MIN_CHARS
        try:
            def fake_call(_messages, **_kwargs):
                ...
                return {
                    ...
                    "female_characters": [{
                        ...
                        "score": 8,
                        "identity": "女主候选",
                        "summary": f"{name}与云烨互动",
                        "is_potential_heroine": True,
                    }],
                }
            ...
            protagonist.HAREM_SCAN_API_DOWNSHIFT_MAX_DEPTH = 1
            protagonist.HAREM_SCAN_API_DOWNSHIFT_MIN_CHARS = 20
            result = protagonist.analyze_chunk_for_heroines(
                "云烨在长安遇见辛月。李安澜随后入场与他对话。" * 20,
                chunk_index=12,
                total_chunks=452,
                max_retries=2,
            )
            ...
            protagonist.HAREM_SCAN_API_DOWNSHIFT_MAX_DEPTH = old_depth
            protagonist.HAREM_SCAN_API_DOWNSHIFT_MIN_CHARS = old_min_chars
    ...
    def test_harem_character_chunk_downshifts_parse_error_by_splitting_text(self):
        calls = []
        old_call = protagonist._call_json_chat_completion
        old_sleep = protagonist.time.sleep
        old_profile = os.environ.get("ANALYSIS_PROFILE")
    ...
    def test_protagonist_chunk_analysis_adds_compact_retry_prompt_after_parse_error(self):
        ...
        self.assertTrue(result["_success"])
        self.assertEqual(len(calls), 2)
        self.assertNotIn("JSON修复重试要求", calls[0][1]["content"])
    ...
    def test_frontend_runtime_config_covers_editable_backend_fields(self):
        ...
        self.assertIn("config.harem_scan_api_downshift_max_depth || '1'", text)
        self.assertIn("configForm.harem_scan_api_downshift_max_depth", text)
        self.assertIn("harem_scan_api_downshift_min_chars: '1200'", text)
        self.assertIn("config.harem_scan_api_downshift_min_chars || '1200'", text)
        self.assertIn("configForm.harem_scan_api_downshift_min_chars", text)
        self.assertIn("<span>后宫降载</span>", text)
        self.assertIn("<span>后宫阈值</span>", text)
        ...
        self.assertIn("typeof storage?.novels?.writable === 'boolean'", text)
        self.assertIn("label: '未知'", text)
        self.assertIn("尚未返回存储写入自检结果", text)
        self.assertIn("danger: !novelsWritable || !resultsWritable", text)
        self.assertIn(":class=\"{ danger: storageStatus.danger }\"", text)
        self.assertIn("function formatStateTime(value)", text)
        self.assertIn("generatedAtText", text)
        self.assertIn("failedHistory", text)
        self.assertIn("<b>历史失败</b>", text)
        self.assertIn("<b>更新</b>", text)
        self.assertIn("const diagnosticsError = ref('')", text)
        ...
        self.assertIn("async function applyState(data, options = {})", text)
        self.assertIn("if (options.refreshDiagnostics !== false)", text)
        self.assertIn("async function refresh(options = {})", text)
        self.assertIn("const stateUnauthorizedNotified = ref(false)", text)
        self.assertIn("stateUnauthorizedNotified.value = false\n    await applyState", text)
        self.assertIn("if (!stateUnauthorizedNotified.value)", text)
        self.assertIn("stateUnauthorizedNotified.value = true", text)
        self.assertIn("stateUnauthorizedNotified.value = false\n  toastSuccess", text)
        ...
        self.assertIn("已加入 ${queued} 本，跳过 ${skipped} 本", text)
        ...
        self.assertIn("ACCESS_TOKEN_QUERY_KEYS", text)
        self.assertIn("const initialUrlParams = new URLSearchParams(window.location.search)", text)
        self.assertIn("ACCESS_TOKEN_QUERY_KEYS.map((key) => initialUrlParams.get(key)?.trim()).find(Boolean)", text)
        self.assertIn("ACCESS_TOKEN_QUERY_KEYS.forEach((key) => cleanUrl.searchParams.delete(key))", text)
    ...
    def test_frontend_api_formats_operation_result_errors(self):
        ...
        self.assertIn("export const ACCESS_TOKEN_QUERY_KEYS = ['token', 'access_token', 'web_access_token']", text)
        self.assertIn("const params = new URLSearchParams(query)", text)
        self.assertIn("ACCESS_TOKEN_QUERY_KEYS.some((key) => (params.get(key) || '').trim())", text)
    ...
    def test_scan_chunk_dimension_boost_reports_truncated_json_without_failing_chunk(self):
        class Message:
            def __init__(self, content):
                self.content = content
    ...
    def test_web_manager_diagnostics_reports_running_task_staleness(self):
        ...
        try:
            ...
            self.assertEqual(diagnostics["generated_at"], "2026-06-09 07:20:00")
    ...
    def test_web_manager_enqueue_many_dedupes_and_reports_skips(self):
        ...
        try:
            ...
            self.assertEqual(result["skipped_reasons"], [
                {"reason": "book already queued or running", "count": 1},

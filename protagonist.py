...
GENERAL_CHARACTER_MAX_CHUNKS = read_int_env(
    ...
    read_int_env("GENERAL_SCAN_MAX_CHUNKS", 0, min_value=0),
    min_value=0,
)
GENERAL_CHARACTER_CONTENT_AWARE_SAMPLING = os.environ.get(
    "GENERAL_CHARACTER_CONTENT_AWARE_SAMPLING",
    os.environ.get("GENERAL_SCAN_CONTENT_AWARE_SAMPLING", "0"),
).strip() == "1"
...
# ── 名字归一化工具（已拆分到 name_normalizer.py） ──
from name_normalizer import (
    normalize_person_name as _normalize_person_name,
    split_multi_names as _split_multi_names,
    split_top_level_name_parts as _split_top_level_name_parts,
    is_group_or_title_enumeration_name as _is_group_or_title_enumeration_name,
    levenshtein_distance as _levenshtein_distance,
    quick_text_similarity as _quick_text_similarity,
)
...
4. 名字相似（如"十六夜"和"黑崎十六夜"）加上 other_names 互相包含，应该合并"""
    ...
    try:
        merge_result = _call_json_chat_completion(
            [
                {"role": "system", "content": system_prompt},
        ...
        logger.error(f"别名合并 LLM 判断失败: {e}")
        return [], []
...
- 普通单相思 → 不列入"""
        ...
        try:
            batch_result = _call_json_chat_completion(
                [
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_prompt},
                ],
